...
Script.prototype.isColdStakingOut = function() {
  ...
  return (buf.length === 51 &&
    buf[2] === Opcode.OP_ROT &&
    buf[4] === Opcode.OP_CHECKCOLDSTAKEVERIFY &&
    buf[5] === 0x14 &&
    buf[27] === 0x14 &&
    buf[49] === Opcode.OP_EQUALVERIFY &&
    buf[50] === Opcode.OP_CHECKSIG);
}
...
Script.prototype.isScriptHashIn = function() {
  ...
  // do not classify unknown script types as invalid
...
Script.prototype.getData = function() {
  ...
  if (this.isColdStakingOut()) {
    return new Buffer(this.chunks[7].buf);
  }
...
/**
 * Builds a scriptSig (a script for an input) that signs a *COLD* public key hash
 * output script.
 *
 * @param {Buffer|string|PublicKey} publicKey
 * @param {Signature|Buffer} signature - a Signature object, or the signature in DER canonical encoding
 * @param {number=} sigtype - the type of the signature (defaults to SIGHASH_ALL)
 */
Script.buildPublicKeyHashInCold = function(publicKey, signature, sigtype) {
  $.checkArgument(signature instanceof Signature || BufferUtil.isBuffer(signature));
  $.checkArgument(_.isUndefined(sigtype) || _.isNumber(sigtype));
  if (signature instanceof Signature) {
    signature = signature.toBuffer();
...
Script.prototype._getOutputAddressInfo = function() {
  ...
  } else if (this.isPublicKeyHashOut() || this.isColdStakingOut()) {
    info.hashBuffer = this.getData();
